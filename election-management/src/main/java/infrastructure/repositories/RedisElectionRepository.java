...
public class RedisElectionRepository implements ElectionRepository {
    ...
    public RedisElectionRepository(RedisDataSource redisData){
        commands = redisData.sortedSet(String.class, String.class);
        pubSubCommands = redisData.pubsub(String.class);
    }
    ...
    public Election sync(Election election) {
        List<ScoredValue<String>> scoredValues = commands.zrangebyscoreWithScores("election:" +election.id(), ScoreRange.from(Integer.MIN_VALUE, Integer.MAX_VALUE));
